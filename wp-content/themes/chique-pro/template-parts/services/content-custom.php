...
<?php
$quantity = get_theme_mod( 'chique_service_number', 3 );

for ( $i = 1; $i <= $quantity; $i++ ) {
	$target = get_theme_mod( 'chique_service_target_' . $i ) ? '_blank' : '_self';

	$link = get_theme_mod( 'chique_service_link_' . $i ) ? get_theme_mod( 'chique_service_link_' . $i ) : '#';
	...
	<article id="services-post-' . esc_attr( $i ) . '" class="hentry"> <div class="hentry-inner">';
		...
		$title   = get_theme_mod( 'chique_service_title_' . $i );
		$content = get_theme_mod( 'chique_service_content_' . $i );
		$more_button = get_theme_mod( 'chique_service_more_button_text_' . $i );

		$image = get_theme_mod( 'chique_service_image_' . $i ) ? get_theme_mod( 'chique_service_image_' . $i ) : '';
