...
if ( ! function_exists( 'chique_posted_on' ) ) :
	...
	function chique_posted_on() {
		...
		$posted_on = sprintf(
			...
			__( '<span class="date-label"> </span>%s', 'chique-pro' ),
		...
		$byline = sprintf(
			...
			__( '<span class="author-label screen-reader-text">By </span>%s', 'chique-pro' ),
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				$output = sprintf( '<span class="cat-links">%1$s%2$s</span>',
					sprintf( _x( '<span class="cat-text screen-reader-text">Categories</span>', 'Used before category names.', 'chique-pro' ) ),
					$categories_list
				); // WPCS: XSS OK.
			}
		}
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">%1$s%2$s</span>',
					sprintf( _x( '<span class="tags-text screen-reader-text">Tags</span>', 'Used before tag names.', 'chique-pro' ) ),
					$tags_list
				); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(*/
						...
						/*__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'chique-pro' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'chique-pro' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'chique_blog_entry_meta_left' ) ) :
	...
	function chique_blog_entry_meta() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( get_theme_mod( 'chique_display_author', 1 ) ) {
				// Get the author name; wrap it in a link.
				printf(
					/* translators: %s: post author */
					__( '<span class="byline"><span class="author-label screen-reader-text">By </span>%s', 'chique-pro' ),
			...
			if ( get_theme_mod( 'chique_display_date', 1 ) ) {
				$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
				if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
					$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
				}
				...
				printf(
					/* translators: %s: post date */
					__( '<span class="posted-on"><span class="date-label"> </span>%s', 'chique-pro' ),
					'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a></span>'
				);
			}

			if ( get_theme_mod( 'chique_display_tags', 0 ) ) {
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list();
				if ( $tags_list ) {
					/* translators: 1: list of tags. */
			...
			if ( get_theme_mod( 'chique_display_categories', 0 ) ) {
				/* translators: used between list items, there is a space after the comma */
				$categories_list = get_the_category_list( ' ' );
				if ( $categories_list ) {
					/* translators: 1: list of categories. */
...
if ( ! function_exists( 'chique_header_title' ) ) :
	...
	function chique_header_title() {
		...
		if ( is_front_page() ) {
			$subtitle = get_theme_mod( 'chique_header_media_subtitle', esc_html__( 'Women Collection', 'chique-pro' ) ) ? '<span class="sub-title">' . wp_kses_post( get_theme_mod( 'chique_header_media_subtitle', esc_html__( 'Women Collection', 'chique-pro' ) ) ) . '</span><!-- .sub-title -->' : '';

			echo wp_kses_post( get_theme_mod( 'chique_header_media_title', esc_html__( 'Spring Fantasy', 'chique-pro' ) ) ) . $subtitle;
		} elseif ( is_singular() ) {
			the_title();
		} elseif ( is_404() ) {
			esc_html_e( 'Oops! That page can&rsquo;t be found.', 'chique-pro' );
		} elseif ( is_search() ) {
			/* translators: %s: search query. */
			printf( esc_html__( 'Search Results for: %s', 'chique-pro' ), '<span>' . get_search_query() . '</span>' );
			...
			echo esc_html( $post_title . $sep . $post_subtitle );
		}
		else {
			the_archive_title();
		}
...
if ( ! function_exists( 'chique_header_text' ) ) :
	...
	function chique_header_text() {
		if ( is_front_page() ) {
			...
			if ( $header_media_url = get_theme_mod( 'chique_header_media_url', esc_html__( '#', 'chique-pro' ) ) ) {
				$target = get_theme_mod( 'chique_header_url_target' ) ? '_blank' : '_self';

				$content .= '<span class="more-button"><a href="'. esc_url( $header_media_url ) . '" target="' . $target . '" class="more-link">' .esc_html( get_theme_mod( 'chique_header_media_url_text', esc_html__( 'Shop Now', 'chique-pro' ) ) ) . '<span class="screen-reader-text">' .wp_kses_post( get_theme_mod( 'chique_header_media_title' , esc_html__( 'Spring Fantasy', 'chique-pro' ) ) ) . '</span></a></span>';
			}
			...
			esc_html_e( 'It looks like nothing was found at this location. Maybe try one of the links below or a search?', 'chique-pro' );
		} elseif( is_search() ) {
...
if ( ! function_exists( 'chique_single_image' ) ) :
	...
	function chique_single_image() {
		...
		$featured_image = get_theme_mod( 'chique_single_layout', 'disabled' );
	...
	function chique_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-body">
				<?php esc_html_e( 'Pingback:', 'chique-pro' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( 'Edit', 'chique-pro' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				...
				<div class="comment-container">
					<div class="comment-header">
						...
						<?php printf( __( '%s <span class="says screen-reader-text">says:</span>', 'chique-pro' ), sprintf( '<cite class="fn author-name">%s</cite>', get_comment_author_link() ) ); ?>
						</header><!-- .comment-meta -->
								...
								<time datetime="<?php comment_time( 'c' ); ?>">
									<?php
										printf(
											...
											esc_html__( '%1$s at %2$s', 'chique-pro' ),
											get_comment_time( get_option( 'date_format' ) ),
											get_comment_time( get_option( 'time_format' ) )
										);
									?>
								</time></a>
							<?php edit_comment_link( esc_html__( 'Edit', 'chique-pro' ), '<span class="edit-link">', '</span>' ); ?>

							<?php if ( '0' == $comment->comment_approved ) : ?>
								<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'chique-pro' ); ?></p>
							<?php endif; ?>
...
function chique_slider_entry_category() {
	...
	if ( 'jetpack-portfolio' === get_post_type( ) ) {
		$meta .= sprintf( '<span class="cat-links">' .'<span class="cat-label screen-reader-text">%1$s</span>%2$s</span>',
			sprintf( _x( 'Categories', 'Used before category names.', 'chique-pro' ) ),
