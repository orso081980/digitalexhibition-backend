...
var viewers_pro_jQuery = defaults_DEARVIEWER.jQuery;
...
FlipBook3D.prototype.updateCSSLayer = function() {
    ...
    viewers_pro_jQuery(viewer.stage.cssRenderer.domElement).find(".df-page-content").css({
        display: "none"
    });
    var isVertical = this.isVertical();
    if (this.leftViewport && pageLeft != null && pageLeft.sheet.element.visible) {
        ...
        if (divLeft != null) {
            ...
            viewers_pro_jQuery(divLeft.element).css({
    ...
    if (this.rightViewPort && pageRight != null && pageRight.sheet.element.visible) {
        ...
        if (divRight != null) {
            ...
            viewers_pro_jQuery(divRight.element).css({
                width: isVertical ? height : width,
                height: isVertical ? width : height,
                top: isVertical ? -this.pageOffset - width / 2 : 0,
                left: isVertical ? 0 : this.pageOffset + width / 2,
...
FlipBook3D.prototype.initSpiral = function() {
    ...
    var box = new THREE.Mesh(new THREE.BoxGeometry(8, 15, 5, 1, 1, 1), new THREE.MeshPhongMaterial({
        color: 0x444444,
        shininess: 0
    }));
    var box2 = new THREE.Mesh(new THREE.BoxGeometry(8, 15, 5, 1, 1, 1), new THREE.MeshPhongMaterial({
        color: 0x444444,
        shininess: 0
    }));
...
FlipBook3D.prototype.set3DCoverNormal = function() {
    ...
    var hasRidge = cover3DType == defaults_DEARVIEWER.FLIPBOOK_COVER_TYPE.RIDGE && !this.isVertical(), grd;
...
FlipBook3D.prototype.refreshSpiral = function() {
    ...
    var scale = this.isVertical() ? Math.max(this.leftHoles.visible ? this.leftSheetWidth : 0, this.rightHoles.visible ? this.rightSheetWidth : 0) : Math.max(this.leftHoles.visible ? this.leftSheetHeight : 0, this.rightHoles.visible ? this.rightSheetHeight : 0);
    scale /= 1000;
...
BaseFlipBookViewer.prototype.isVertical = function() {
    return this.app.options.calendarMode === true;
};
BaseViewer.prototype.afterConstructor = function() {
    if (this.app.options.calendarMode === true) {
...
var common_pro_jQuery = defaults_DEARVIEWER.jQuery;
...
defaults_DEARVIEWER.openAttachmentLightBoxes = function() {
    ...
    var attachments = common_pro_jQuery("body .df-auto-open-lightbox");
    if (attachments.length > 0) {
        defaults_DEARVIEWER._isHashTriggered = true;
        common_pro_jQuery(attachments[0]).trigger("click");
        defaults_DEARVIEWER._isHashTriggered = false;
    }
    //Deeplink on same page #392
    common_pro_jQuery(window).on('hashchange', function(e) {
        var hash = location.hash;
        if (hash && hash.length > 5) {
            defaults_DEARVIEWER.hashFocusBookFound = false;
            utils.detectHash();
            if (defaults_DEARVIEWER.hashFocusBookFound === true && window.location.hash.indexOf("/_") === -1) {
                //since the trigger are from internal hash chnages like table of content. To trigger again we need to add a small difference so next trigger also works
                history.replaceState(undefined, undefined, window.location.href + "_");
            }
        }
    });
};
...
PDFDocumentProvider.prototype.renderAutoLinks = function(autoLinkItems, textData, element, viewport) {
    if (autoLinkItems && autoLinkItems.length > 0) {
        var divAutoLink = common_pro_jQuery(element).siblings(".df-auto-link-content");
        if (divAutoLink.length === 0) {
            divAutoLink = common_pro_jQuery('<div class="df-auto-link-content">');
            common_pro_jQuery(element).parent().prepend(divAutoLink);
        }
        divAutoLink.html("");
     ...
     */ function(page) {
        ...
        page.getAnnotations().then(function(annotationsData) {
            ...
            if (app.options.annotationClass && app.options.annotationClass !== "") {
                common_pro_jQuery(element).find(" > section").addClass(app.options.annotationClass);
            }
            if (provider.PDFLinkItemsCache[pageNumberToRender] == void 0) {
                var pdfLinks = "," + common_pro_jQuery(element).find("a:not([href^='#'])").map(function() {
                    return common_pro_jQuery(this).attr("href");
                }).get().join(",") + ",";
...
//dearpdf compatibility
window.dearPDFOpenFileOptions = defaults_DEARVIEWER.openFileOptions;
...
dflip_pro_jQuery(document).ready(function() {
    ...
    body.on('click', '.df-trigger', function(event) {
        ...
        return false; //incase the trigger is a link elements and has hash destination it will trigger a popsate.
    });
